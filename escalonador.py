...
class Escalonador:
    ...
    def executar(self):
        while not self.todos_processos_concluidos():
            ...
            if all(p.creditos == 0 for p in self.processos):
    ...
    def obter_proximo_processo(self):
        ...
        for processo in self.processos:
            if processo.estado not in [EstadoProcesso.BLOCKED, EstadoProcesso.EXIT]:
    ...
    def redistribuir_creditos(self):
        for processo in self.processos:
            ...
            if processo.estado == EstadoProcesso.BLOCKED and processo.tempo_bloqueado_restante == 0:
